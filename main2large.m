...
set(gcf, 'position', [100,100,1000,1.4*6.3*10])
matrix = zeros(N,N);

for x = 0:1:length(matrix(1,:))-1
    sol = ifft(exp(Der.*100.*x./N).*fft(phigrid),'symmetric')-zeta+kxgrid*100*x/N;
    matrix(:,x+1) = cos(sol);
end
% 512 x-values that range from 0 to 100
% 512 zeta-values that range from 0 to 2*pi.
...
Yticks = {};
for i = 1:length(A)
    Xticks{i} = (length(matrix(1,:))/10)*i/length(A)/.512;
end
xticklabels(Xticks);

for i = 1:length(B)
    Yticks{i} = round(3*pi-pi*i,1);
end
yticklabels(Yticks);
