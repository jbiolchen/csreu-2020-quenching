...
if var == 0
  ...
  Der = var/2-sqrt(var^2/4+(ell.^2)*const^2+k_x*var*1i*ell-1);
end
...
Der_kx = (var*1i*ell)./((Der-1).^2.*2.*sqrt(var^2/4+(ell.^2)*const^2+k_x*var*1i*ell-1)); % d/dkx (Der-1).^(-1)
dkx_F0 = ifft(Der_kx.*fft(phi-h)*dk_x + (Der-1).^(-1).*fft(dk_x), 'symmetric'); % (dF0/dk_x)*dk_x

Der_var = (1/2 - (var/2+k_x*1i*ell)./(2*sqrt(var^2/4+(ell.^2)*const^2+k_x*var*1i*ell-1)))./((Der-1).^2); % d/dvar (Der-1).^(-1)
